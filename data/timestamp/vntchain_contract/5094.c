...
// 10%(奖金比例)
KEY uint256 fee = U256(11);

KEY address creator;

KEY uint256 freeA = U256(110000000000000000000); // 100*10**18;
...
constructor a5094(){       // Payable函数可以接收来自交易中的原生代币，使用符号$表示
    creator = GetSender();   // owner即交易发起者的地址
    total = 0;    // 游戏总局数
}
...
void checkOwner()
{
    ...
    Require(Equal(sender, creator) == true, "Only the creator can operate");
}
...
void GetFreeChips()
{
    ...
    Accounts.value.balance = U256SafeAdd(Accounts.value.balance, freeA);
    deposit = U256SafeAdd(deposit, freeA);
...
// 获取交易发起人
UNMUTABLE
address GetOwner() { return creator; }
...
// 获取游戏的总局数
UNMUTABLE
uint64 GetTotalGameCount() { return total; }
